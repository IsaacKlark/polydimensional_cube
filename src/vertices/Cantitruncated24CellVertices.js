const Cantitruncated24CellVertices = (
  dimensions,
  DimensionOfFigure,
  setVerticesArray,
  scale,
  ...
  const baseGroup1 = [17, 17 * (1 + Math.sqrt(2))];
  const baseGroup2 = [0, 17 * (2 + Math.sqrt(2))];

  for (let i = 2; i < +DimensionOfFigure; i++) {
    if (i === 3) {
      baseGroup1.push(17 * (1 + 2 * Math.sqrt(2)));
      baseGroup2.push(17 * (2 + i * Math.sqrt(2)));
    } else {
      baseGroup1.push(17 * (3 + 3 * Math.sqrt(2)));
      baseGroup2.push(17 * (2 + i * Math.sqrt(2)));
    }
  }
...
export default Cantitruncated24CellVertices;
