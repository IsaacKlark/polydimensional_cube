const Bitruncated24CellVertices = (
  dimensions,
  DimensionOfFigure,
  setVerticesArray,
  scale,
  ...
  const baseGroup1 = [0, 20 * (2 + 2*Math.sqrt(2))];
  const baseGroup2 = [20, 20 * (3 + 2*Math.sqrt(2))];

  for (let i = 2; i < +DimensionOfFigure; i++) {
    baseGroup1.splice(1, 0, 20 * (2 + Math.sqrt(2)));
    baseGroup2.splice(1, 0, 20 * (1 + Math.sqrt(2)));
  }
...
export default Bitruncated24CellVertices;
