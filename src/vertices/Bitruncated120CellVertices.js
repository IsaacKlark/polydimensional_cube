const Bitruncated120CellVertices = (
  dimensions,
  DimensionOfFigure,
  setVerticesArray,
  scale,
  ...
  const fi = 1.618;
  const length = 12;
  const baseGroup1 = [0, 0, 4 * fi, 2 * fi ** 4].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup2 = [2 + fi, 2 + fi, 2 + 5 * fi, 2 + 5 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup3 = [0, 1, 4 + 5 * fi, 1 + 5 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup4 = [0, 1, 3 * fi, 3 + 7 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup5 = [0, fi, 5 * fi ** 2, 1 + 4 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup6 = [0, 3 * fi ** 2, 3 + 4 * fi, 4 + 3 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup7 = [1, 2 * fi, 3 + 7 * fi, 2 + fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup8 = [1, 2 * fi, 5 * fi ** 2, fi ** 4].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup9 = [1, 2, fi ** 5, 2 + 5 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup10 = [1, 2 * fi ** 2, fi ** 5, 4 + 3 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup11 = [fi, 2, fi ** 3, 3 + 7 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup12 = [fi, fi ** 4, 2 + 5 * fi, 4 + 3 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup13 = [2 * fi, 1 + 4 * fi, 2 + 5 * fi, 3 * fi ** 2].map(
    (number) => Math.trunc(number * length)
  );
  const baseGroup14 = [2 * fi, fi ** 4, 1 + 5 * fi, 3 + 4 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup15 = [2, 2 * fi, 2 * fi ** 4, 2 * fi ** 2].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup16 = [2, fi ** 3, 4 + 5 * fi, 3 * fi ** 2].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup17 = [2, 2 + fi, fi ** 5, 3 + 4 * fi].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup18 = [fi ** 3, 4 * fi, fi ** 5, fi ** 4].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup19 = [fi ** 3, 2 * fi ** 2, 1 + 5 * fi, 2 + 5 * fi].map(
    (number) => Math.trunc(number * length)
  );
  const baseGroup20 = [3 * fi, 2 * fi ** 2, 1 + 4 * fi, fi ** 5].map((number) =>
    Math.trunc(number * length)
  );
  const baseGroup21 = [2 + fi, fi ** 3, 5 * fi ** 2, 2 * fi ** 2].map(
    (number) => Math.trunc(number * length)
  );
  const baseGroup22 = [2 + fi, 3 * fi, 4 + 5 * fi, fi ** 4].map((number) =>
    Math.trunc(number * length)
  );
...
export default Bitruncated120CellVertices;
