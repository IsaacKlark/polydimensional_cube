const Cantellated24CellVertices = (
  dimensions,
  DimensionOfFigure,
  setVerticesArray,
  scale,
  ...
  const baseGroup1 = [0, 30*(2 + 2*Math.sqrt(2))];
  const baseGroup2 = [30, 30*(1 + 2*Math.sqrt(2))];

  for (let i = 2; i < +DimensionOfFigure; i++) {
      baseGroup1.splice(1, 0, 30 * Math.sqrt(2));
      baseGroup2.splice(1, 0, 30 * (1 + Math.sqrt(2)));
  }
...
export default Cantellated24CellVertices;
