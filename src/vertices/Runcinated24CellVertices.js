const Runcinated24CellVertices = (
  dimensions,
  DimensionOfFigure,
  setVerticesArray,
  scale,
  ...
  const baseGroup1 = [0, 0];
  const baseGroup2 = [40, 40];

  for (let i = 2; i < +DimensionOfFigure; i++) {
    if (i === +DimensionOfFigure - 1) {
      baseGroup1.push(40 * (2 + Math.sqrt(2)));
      baseGroup2.push(40 * (1 + Math.sqrt(2)));
    } else {
      baseGroup1.push(40 * Math.sqrt(2));
      baseGroup2.push(40 * (1 + Math.sqrt(2)));
    }
  }
...
export default Runcinated24CellVertices;
