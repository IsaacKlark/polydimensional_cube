const Truncated120CellVertices = (
  dimensions,
  DimensionOfFigure,
  setVerticesArray,
  scale,
  ...
  const baseGroup1 = [1, 3+4*fi, 3+4*fi, 3+4*fi].map((number) => Math.trunc(number * length));
  const baseGroup2 = [fi**3, fi**3, fi**3, 5+6*fi].map((number) => Math.trunc(number * length));
  const baseGroup3 = [2*fi**2, 2*fi**2, 2*fi**2, 2*fi**4].map((number) => Math.trunc(number * length));
  const baseGroup4 = [0, 1, 4+5*fi, fi**5].map((number) => Math.trunc(number * length));
  const baseGroup5 = [0, 1, 4+7*fi, 1+3*fi].map((number) => Math.trunc(number * length));
  const baseGroup6 = [0, fi**2, 3*fi**3, 2+5*fi].map((number) => Math.trunc(number * length));
  const baseGroup7 = [0, fi**2, 5+6*fi, fi**4].map((number) => Math.trunc(number * length));
  const baseGroup8 = [0, 2*fi, 2*fi**4, 2*fi**3].map((number) => Math.trunc(number * length));
  const baseGroup9 = [1, fi**2, 4+7*fi, 2*fi**2].map((number) => Math.trunc(number * length));
  const baseGroup10 = [1, fi**3, 3*fi**3, 3+4*fi].map((number) => Math.trunc(number * length));
  const baseGroup11 = [fi**2, 2*fi, 4+7*fi, fi**3].map((number) => Math.trunc(number * length));
  const baseGroup12 = [fi**2, 2*fi**2, 4+5*fi, 3+4*fi].map((number) => Math.trunc(number * length));
  const baseGroup13 = [fi**2, 2*fi**3, 2+5*fi, 3+4*fi].map((number) => Math.trunc(number * length));
  const baseGroup14 = [2*fi, fi**4, fi**5, 3+4*fi].map((number) => Math.trunc(number * length));
  const baseGroup15 = [fi**3, 2*fi**2, fi**5, 2+5*fi].map((number) => Math.trunc(number * length));
  const baseGroup16 = [fi**3, 1+3*fi, 4+5*fi, 2*fi**3].map((number) => Math.trunc(number * length));
  const baseGroup17 = [2*fi**2, 1+3*fi, 3*fi**3, fi**4].map((number) => Math.trunc(number * length));
...
export default Truncated120CellVertices;
